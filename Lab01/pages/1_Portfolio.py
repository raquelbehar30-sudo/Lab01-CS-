...
def links_section():
    ...
    linkedin_link = f'<a href = "{info.my_linkedin_url}"><img src = "{info.linkedin_image_url}" alt="LinkedIN" width = "75" ></a>'
...
def skills_section(skills_data, spoken_data):
    st.header("Skills")
    st.subheader("Hard/Soft")
    for skill, percentage in skills_data.items():
        st.write(f"{skill}{info.skills_icons.get(skill,)}")
